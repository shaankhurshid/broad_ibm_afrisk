...
vs$score_decile <- classifier(risk=vs$pred5,ncuts=10)
vs$charge_decile <- classifier(risk=vs$charge.pred5,ncuts=10)

calib_score <- GND.calib(pred=vs$pred5,tvar=vs$af_5y_sal.t,out=vs$af_5y_sal,groups=vs$score_decile,
                   cens.t=vs$censored,adm.cens=5)

calib_charge <- GND.calib(pred=vs$charge.pred5,tvar=vs$af_5y_sal.t,out=vs$af_5y_sal,groups=vs$charge_decile,
                         cens.t=vs$censored,adm.cens=5)
