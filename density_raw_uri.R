...
incident_af <- mydata[is_af==1]
no_incident_af <- mydata[is_af==0]

##### PLOT 3: EHR-AF
# Generate score distribution
x <- list(v1=incident_af$EHR_AF,v2=no_incident_af$EHR_AF)
data <- melt(x)
...
ggplot() + geom_density(data=data,aes(x=value,fill=L1),alpha=0.55) +
  ...
  scale_y_continuous(breaks=seq(0,0.50,0.10),expand=c(0,0),limits=c(0,0.50)) +  # modify y axis limits as needed
...
ggsave(filename='ehr_density.pdf',height=2.5,width=3.5,units='in',scale=4,device='pdf')
